...
(define (waydroid-shepherd-service config)
  (list (shepherd-service (provision '(waydroid))
                          (requirement '(loopback dbus-system))
                          (documentation "Launch the waydroid container")
                          (start #~(make-forkexec-constructor (list (string-append #$waydroid
                                                                     "/bin/waydroid")
                                                               "container"
                                                               "start")
...
(define (wine-binfmt-service config)
  (list (shepherd-service (provision '(wine-binfmt))
                          ...
                          (stop #~(lambda _
                                    (begin
                                      (delete-file
                                       "/proc/sys/fs/binfmt_misc/wine64") #t))))))

(define-public wine-binfmt-service-type
  (service-type (name 'wine-binfmt)
