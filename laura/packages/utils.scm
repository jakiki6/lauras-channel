  ...
  (package
    ...
    (source
      (origin
        (method git-fetch)
        (uri (git-reference
              (url "https://github.com/cetus9/u8_r2_plugin")
              (commit "7bedc13")))
        (file-name (git-file-name name version))
(patches (search-patches
                          "laura/packages/patches/u8_r2_plugin.patch"))
        (sha256 (base32 "0affzh6c9ybzircmsvqahvryspc32nq7n3f3pqa6kql8zp7az6gf"))))
    (build-system gnu-build-system)
    (native-inputs (list pkg-config))
    (inputs (list radare2 openssl))
    (arguments (list #:tests? #f
#:phases #~(modify-phases %standard-phases
(replace 'configure (lambda _
  (setenv "CC" "gcc")))
(replace 'install (lambda _
(mkdir-p (string-append #$output "/share/radare2/plugins"))
(copy-file "asm_u8.so" (string-append #$output "/share/radare2/plugins/asm_u8.so"))
(copy-file "anal_u8.so" (string-append #$output "/share/radare2/plugins/anal_u8.so"))
)))))
    (home-page "https://github.com/cetus9/u8_r2_plugin")
    (synopsis "radare2 plugins for nX-U8/100")
    (description "Experimental radare2 disassembly and analysis plugins for nX-U8/100 architecture.")
    (license license:lgpl2.1)))
